...
# Usage: python3 complete_test_generate.py -d Dataset/chem -r Dataset/chem/test_results --preset synthesizer/presets/chem.json --checkpoint <path_to_checkpoint>
# Example: python3 complete_test_generate.py -d Dataset/mini_sample -r Dataset/mini_sample/test_results --preset synthesizer/presets/chem.json --checkpoint "weights/logs_chemistry/tacotron_model.ckpt-159000"  
...
class Generator(object):
	...
	# Returns an np array of T RGB images that are each H x W in dimension (total of Time x Height x Width x 3 dimensions)
...
if __name__ == '__main__':
	...
	for vid in tqdm(videos): # Note tqdm generates progress bar
		...
		for (ss, es), images in tqdm(contiguous_window_generator(vidpath)):
			...
			command = template.format(ss, vidpath + 'audio.wav', es, '{}{}_{}:{}.wav'.format(GTS_ROOT, vidname, ss, es))
